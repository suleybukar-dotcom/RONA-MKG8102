...
Cinq pages de référence couvrant les cas difficiles réellement rencontrés, avec le comportement attendu de la [Skill v2](../SKILL.md). La Skill se réexécute sur ce jeu après chaque révision et chaque changement de modèle ; les écarts sont examinés avant remise en production.

> **État : défini, jamais réexécuté.** Aucune colonne « résultat » n'est renseignée : aucun résultat n'est inventé. Les valeurs de prix changent d'une semaine à l'autre : le test vérifie les **règles** (dates, extrait, divergences, signalements, statut), non les montants. Les fiches de référence citées existent dans la base sous le statut « Validée » (pages 2 à 4) ou proviennent d'un cas documenté dans le champ « Biais connu » (pages 1 et 5).

Réglages communs : Skill v2, KIQ centrale, échelle de fiabilité de l'équipe, connecteur de récupération indiqué. Pour chaque page, vérifier aussi : statut « À valider », champ « Validé par » vide, motif de rejet vide, une ligne de Journal produite même à zéro élément.
...
| Résultat de réexécution | Non réexécuté |
